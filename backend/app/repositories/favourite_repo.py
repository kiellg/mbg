...
def remove_favourite(user_id: str, target_id: str) -> bool:
    """Remove a restaurant or menu item from the user's favourites"""
    for i, record in enumerate(FAVOURITES):
        if record["user_id"] == user_id and record["target_id"] == target_id:
            FAVOURITES.pop(i)
            return True
    return False
...
def is_favourite(user_id: str, target_id: str) -> bool:
    """Check if a restaurant or menu item is favourited by a user"""
    return any(
        r["user_id"] == user_id and r["target_id"] == target_id
        for r in FAVOURITES
    )
