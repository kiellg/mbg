...
def get_order_record(order_id: int) -> Optional[Dict[str, Any]]:
    """Simulate fetching an order record from the database."""
    return _ORDERDB.get(order_id)


def list_order_records() -> List[Dict[str, Any]]:
    """Return all order records sorted by order_id."""
    return [_ORDERDB[oid] for oid in sorted(_ORDERDB.keys())]
...
def create_order_record(
    ...
    _ORDERDB[order_id] = order_record
    return order_record
...
def delete_order_record(order_id: int) -> bool:
    ...
    if order_id not in _ORDERDB:
        return False
    del _ORDERDB[order_id]
    return True